...
def handleXls(excel_url: str, verbose=True) -> pd.DataFrame:
    ...
    # 删除存活天数小于0的行
    if verbose:
        print("存活天数<=0的行数：", xls[xls['days'] <= 0].shape[0])
    xls = xls[xls['days'] > 0]
    if verbose:
...
class TCGA_dataset(Dataset):
    def __init__(self,
                 folderPath: str = rp(r"G:/LUSC"),
                 folderList: list = None,
                 excel_url: str = rp(r"LUSC/survival.xlsx"),
                 mRNA_url: str = rp(r"LUSC/LUSC.mRNAseq.xlsx"),
                 miRNA_url: str = rp(r"LUSC/LUSC.miRNAseq.xlsx"),
                 k_nebs: int = 20,
